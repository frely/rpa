...
def getcheckindata(headers):
    ...
    if response.status_code != 200:
        ...
        if response.status_code != 200:
            ...
            response = requests.post(url, headers=headers, data=data)
            if response.status_code != 200:
                logging.error("请求接口失败", response.json())
                sys.exit(1)
    else:
...
def run():
    ...
    logging.info("查询日期范围: %s %s", check_start_time, check_end_time)
    ...
    for i in list:
        ...
        logging.info('当前查询考勤公司: %s', i['name'])
    ...
    for i in list:
        ...
        logging.info('当前查询晨会企微直播信息公司: %s', i['name'])
        living_id_dict = get_living_id(headers)
        if len(living_id_dict) == 1:
            logging.info("%s: 查询到直播记录", i['name'])
            dict = {}
    ...
    if len(living_id_list_num) == 0:
        logging.info('当天未查询到直播记录')
        sys.exit(0)
    elif len(living_id_list_num) > 1:
        logging.error('当天查询到多个直播记录')
        sys.exit(1)
    else:
        ...
        if len(watch_list) == 0:
            ...
            for i in watch_list:
                ...
                if i['user_name'] not in getcheckindata_dict:
                    try:
                        logging.info('%s %s未参加晨会', i['user_name'], getcheckindata_dict[i['user_name']]['部门'])
                        df_department_list.append(getcheckindata_dict[i['user_name']]['部门'])
                        df_type_list.append('未参加晨会')
                        df_watch_time_list.append('')
                    except:
                        logging.info('%s 不在部门信息中', i['user_name'])
                        df_department_list.append('')
                        df_type_list.append('未参加晨会')
                        df_watch_time_list.append('')
                else:
                    if getcheckindata_dict[i['user_name']]['迟到时长'] != '--':
                        logging.info('%s %s 迟到: %s 观看时间: %s',i['user_name'], getcheckindata_dict[i['user_name']]['部门'], getcheckindata_dict[i['user_name']]['迟到时长'], i['watch_time'])
                        df_department_list.append(getcheckindata_dict[i['user_name']]['部门'])
                        df_type_list.append('迟到' + getcheckindata_dict[i['user_name']]['迟到时长'])
                        df_watch_time_list.append(i['watch_time'])
                    else:
                        logging.info('%s %s 观看时间: %s',i['user_name'], getcheckindata_dict[i['user_name']]['部门'], i['watch_time'])
                        df_department_list.append(getcheckindata_dict[i['user_name']]['部门'])
